...
/*					 - prevented NULL pointer from causing an error */
...
void MemTrackerFinalReport()
{
  ...
  if(_hashtable.total_bytes == 0) {
    ...
    if(_hashtable.error_array != NULL) {
      printf("HOWEVER, memory corruptions has been detected!\n");
    }
  }
  if(_hashtable.error_array != NULL) {
  ...
  printf("\n - Memory report done, press any key. - \n");
  getc(stdin);
  return;
}
const char * getTLA(char *TLA) {
...
{
	if (_mt_src_func == NULL || _mt_src_file == NULL)
		return;
